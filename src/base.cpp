...
namespace shiny {
    ...
    Encoded Base::encodeVec(std::string str) const {
        ...
        const std::size_t substrLen = (sizeof(uint64_t) * 8) / bitCount;
        const std::size_t zeroPaddingCount = (sizeof(uint64_t) * 8) % bitCount;
        ...
        if (zeroPaddingCount != 0) {
            ...
            for (std::size_t i = 0; i < nums.first.size(); i++) {
                ...
                if (i + 1 != nums.first.size()) temp.first.back() |= nums.first.at(i + 1) >> ((sizeof(uint64_t) * 8) - (4 * (2 + ((i - min) % 15))));
            }
            return temp;
        }
        return nums;
    ...
    std::string Base::decodeVec(const Encoded &nums) const {
        ...
        if (zeroPaddingCount != 0) {
            for (std::size_t i = 0; i < nums.first.size(); i++) {
                if (i % (sizeof(uint64_t) * 8 / zeroPaddingCount - 1) == 0) { temp.first.push_back(nums.first.at(i) >> 4); }
                if (i % (sizeof(uint64_t) * 8 / zeroPaddingCount - 1) == (sizeof(uint64_t) * 8 / zeroPaddingCount - 2)) {
                    temp.first.push_back(nums.first.at(i) & BITMASK((sizeof(uint64_t) * 8) - zeroPaddingCount));
                    continue;
                };

                temp.first.push_back(0);
                temp.first.back() |= nums.first[i] << (sizeof(uint64_t) * 8 - zeroPaddingCount * (2 + (i % 15)));
                temp.first.back() |= nums.first[i + 1] >> (zeroPaddingCount * (2 + (i % 15)));
                temp.first.back() &= BITMASK((sizeof(uint64_t) * 8) - zeroPaddingCount);
            }
        }
    ...
    const Base ascii({(char)0,   (char)1,   (char)2,   (char)3,   (char)4,   (char)5,   (char)6,   (char)7,   (char)8,   (char)9,   (char)10,  (char)11,  (char)12,  (char)13,  (char)14,  (char)15,
